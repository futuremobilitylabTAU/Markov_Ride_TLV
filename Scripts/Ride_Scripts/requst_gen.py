...
path_das=r'C:\Users\dadashev\Dropbox\Optimizing_Mobility_with_Markovian_Model_for_AMoD\Data\Demand\Activity_schedule\Das.csv'
path_centroid_circle=r'C:\Users\dadashev\Dropbox\Optimizing_Mobility_with_Markovian_Model_for_AMoD\Data\Keys\section_coordination_by_taz.csv'

## Save the result
outputs_folder=r'C:\Users\dadashev\Dropbox\Optimizing_Mobility_with_Markovian_Model_for_AMoD\Data\Demand\Amod_schedule\r.json'
...
das['x_y_random_origins'] = das.apply(lambda x: {'x': float(x['or']['X'].iloc[0]), 'y': float(x['or']['Y'].iloc[0])}, axis=1)
das['x_y_random_dest'] = das.apply(lambda x: {'x': float(x['des']['X'].iloc[0]), 'y': float(x['des']['Y'].iloc[0])}, axis=1)
