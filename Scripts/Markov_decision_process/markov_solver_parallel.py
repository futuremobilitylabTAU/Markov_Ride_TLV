...
class MarkovDecisionProcess:
    ...
    def __init__(self, Transitions, Reward, gamma):
        ...
        self.len_transition=len(Transitions)
...
def worker_function(worker_id, m, shared_dict, states, shared_policy, shared_delta):
    ...
    for s in states:
        ...
        for a in m.actions(s):
            ...
            for t in trans:
                intermediate_reward = float(m.reward[(m.reward['State'] == s) & (m.reward['Action'] == a)]['Reword'].iloc[0])
...
def main():
    ...
    Transitions = pd.read_csv(r'C:\Users\dadashev\Dropbox\Optimizing_Mobility_with_Markovian_Model_for_AMoD\Data\Markov_decision_process\model_7\Transitions\trans_df.csv')
    Reward = pd.read_csv(r'C:\Users\dadashev\Dropbox\Optimizing_Mobility_with_Markovian_Model_for_AMoD\Data\Markov_decision_process\model_7\Reword\rew_df.csv')
    gamma =0.9
#/home/bathen/Future Mobility Lab Dropbox/Future Mobility Lab TAU
#C:\Users\dadashev\Future Mobility Lab Dropbox\Future Mobility Lab TAU\
    ...
    with Manager() as manager:
        ...
        while m.delta > epsilon:
            ...
            pi_df.to_csv(r"C:\Users\dadashev\Dropbox\Optimizing_Mobility_with_Markovian_Model_for_AMoD\Data\Markov_decision_process\model_7\Outputs\optimal_VI_%s.csv"%(count), index=False)
            v_df.to_csv(r"C:\Users\dadashev\Dropbox\Optimizing_Mobility_with_Markovian_Model_for_AMoD\Data\Markov_decision_process\model_7\Outputs\optimal_state_value_%s.csv"%(count), index=False)
    m.elapsed_time = time.time() - start_time
    print(m.elapsed_time,m.len_transition)
    return (m)
...
if __name__ == "__main__":
  ...
  #  pi_df = pd.DataFrame(list(m.pi.items()), columns=['State', 'Action'])
   # v_df = pd.DataFrame(list(m.V1.items()), columns=['State', 'Value'])
    ...
    #pi_df.to_csv(r"/home/bathen/Dropbox/Optimizing_Mobility_with_Markovian_Model_for_AMoD/Data/Markov_decision_process/model_5/Outputs/optimal_VI.csv", index=False)
   # v_df.to_csv(r"/home/bathen/Dropbox/Optimizing_Mobility_with_Markovian_Model_for_AMoD/Data/Markov_decision_process/model_5/Outputs/optimal_state_value.csv", index=False)
