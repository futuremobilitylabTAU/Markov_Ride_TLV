    ...
    def set_up_transition(self):
        ...
        for trans in self.next_state_from_state:
            ...
            if trans[0][1][0]=='T':  
                ...
                supply=self.supply_info[(self.supply_info['Origin Cluster']==area_or)].copy()
            ...
            if trans[0][1][0]=='R':
                ...
                supply=self.supply_info[(self.supply_info['Origin Cluster']==area_or)&(self.supply_info['Destenation Cluster']==area_dest)].copy()
                supply=supply[supply['Electric_sum']>0].copy()
                mu, loc, sigma = scipy.stats.lognorm.fit(supply['Electric_sum'], floc=0)
                sigma=abs(sigma)       
                m=self.enregy.number_of_energy_levels
...
requst=pd.read_json(r'C:\Users\dadashev\Dropbox\Optimizing_Mobility_with_Markovian_Model_for_AMoD\Data\Demand\Amod_schedule\requst_op.json')
#%%
...
data=MDP_Data(192,13,requst,taz_info,trajectory)
